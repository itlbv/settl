package com.itlbv.settl.mobs.util;

public class MobConstants {
    public static final float MOB_SPEED = 4f;
    public static final float MOB_RENDER_WIDTH = 2.5f;
    public static final float MOB_RENDER_HEIGHT = 2.5f;
    public static final float MOB_BODY_RADIUS = .3f;
    public static final float MOB_SENSOR_RADIUS = 1f;

    /*
    public static final float HUMAN_SPEED = 4f;
    public static final float HUMAN_WIDTH = 2.5f;
    public static final float HUMAN_HEIGHT = 2.5f;
    public static final float HUMAN_BODY_WIDTH = .6f;
    public static final float HUMAN_BODY_HEIGHT = .4f;
    public static final float HUMAN_SENSOR_WIDTH = 2f;
    public static final float HUMAN_SENSOR_HEIGHT = 2f;

    public static final float ORC_SPEED = 5f;
    public static final float ORC_WIDTH = 2.5f;
    public static final float ORC_HEIGHT = 2.5f;
    public static final float ORC_BODY_WIDTH = .8f;
    public static final float ORC_BODY_HEIGHT = .6f;
    public static final float ORC_SENSOR_WIDTH = 2f;
    public static final float ORC_SENSOR_HEIGHT = 2f;
    */
}
